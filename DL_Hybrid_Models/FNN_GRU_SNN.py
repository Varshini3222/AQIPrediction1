...
print("="*70)
print("DEEP LEARNING HYBRID: FNN + GRU + SNN")
print("="*70)
...
hybrid_pred = (fnn_pred + gru_pred + snn_pred) / 3
...
plt.title('Confusion Matrix - FNN + GRU + SNN\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_dl_fnn_gru_snn.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'fnn': fnn_model, 'gru': gru_model, 'snn': snn_model},
           'hybrid_dl_fnn_gru_snn.joblib')