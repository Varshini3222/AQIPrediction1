...
from tensorflow.keras.layers import GRU, Conv1D, Flatten, Dense, Input
import joblib
...
print("="*70)
print("DEEP LEARNING HYBRID: GRU + CNN + SNN")
print("="*70)
...
hybrid_pred = (gru_pred + cnn_pred + snn_pred) / 3
...
plt.title('Confusion Matrix - GRU + CNN + SNN\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_dl_gru_cnn_snn.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'gru': gru_model, 'cnn': cnn_model, 'snn': snn_model},
           'hybrid_dl_gru_cnn_snn.joblib')