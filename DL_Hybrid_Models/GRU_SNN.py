...
print("="*70)
print("DEEP LEARNING HYBRID: GRU + SNN (SlideNN)")
print("="*70)
...
hybrid_pred = (gru_pred + snn_pred) / 2
...
plt.title('Confusion Matrix - GRU + SNN\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_dl_gru_snn.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'gru': gru_model, 'snn': snn_model},
           'hybrid_dl_gru_snn.joblib')