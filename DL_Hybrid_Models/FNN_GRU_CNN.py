...
print("="*70)
print("DEEP LEARNING HYBRID: FNN + GRU + CNN")
print("="*70)
...
hybrid_pred = (fnn_pred + gru_pred + cnn_pred) / 3
...
plt.title('Confusion Matrix - FNN + GRU + CNN\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_dl_fnn_gru_cnn.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'fnn': fnn_model, 'gru': gru_model, 'cnn': cnn_model},
           'hybrid_dl_fnn_gru_cnn.joblib')