...
from tensorflow.keras.layers import Dense, GRU, Conv1D, Flatten, Input
import joblib
...
print("="*70)
print("DEEP LEARNING HYBRID: FNN + GRU + CNN + SNN")
print("="*70)
...
fnn_model.fit(X_train, y_train, epochs=20, batch_size=32, verbose=0)
...
gru_model.fit(X_train_seq, y_train, epochs=20, batch_size=32, verbose=0)
...
hybrid_pred = (fnn_pred + gru_pred + cnn_pred + snn_pred) / 4
...
plt.title('Confusion Matrix - FNN + GRU + CNN + SNN\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_dl_fnn_gru_cnn_snn.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'fnn': fnn_model, 'gru': gru_model, 'cnn': cnn_model, 'snn': snn_model},
           'hybrid_dl_fnn_gru_cnn_snn.joblib')