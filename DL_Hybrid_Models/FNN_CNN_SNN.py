...
print("="*70)
print("DEEP LEARNING HYBRID: FNN + CNN + SNN")
print("="*70)
...
hybrid_pred = (fnn_pred + cnn_pred + snn_pred) / 3
...
plt.title('Confusion Matrix - FNN + CNN + SNN\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_dl_fnn_cnn_snn.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'fnn': fnn_model, 'cnn': cnn_model, 'snn': snn_model},
           'hybrid_dl_fnn_cnn_snn.joblib')