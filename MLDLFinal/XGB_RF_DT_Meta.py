# Hybrid_Combo1_XGB_RF_with_DT_Meta.py
import pandas as pd
import numpy as np
...
print("🚀 HYBRID COMBO 1: (XGBoost + Random Forest) → Decision Tree as Meta")
print("=" * 70)
...
meta_features_train = np.column_stack([xgb_pred_train, rf_pred_train])
meta_features_test = np.column_stack([xgb_pred_test, rf_pred_test])

print(f"Meta features shape: {meta_features_train.shape}")

# Step 5: Train Meta Model (Decision Tree)
print("🎯 Training meta model (Decision Tree)...")
meta_model = DecisionTreeRegressor(max_depth=8, random_state=42)
...
combo1_metrics = calculate_all_metrics(y_test, final_pred, "Hybrid Combo 1")
...
xgb_metrics = calculate_all_metrics(y_test, xgb_pred_test, "XGBoost Alone")
rf_metrics = calculate_all_metrics(y_test, rf_pred_test, "Random Forest Alone")
dt_alone = DecisionTreeRegressor(max_depth=8, random_state=42).fit(X_train, y_train)
dt_pred_alone = dt_alone.predict(X_test)
dt_metrics = calculate_all_metrics(y_test, dt_pred_alone, "Decision Tree Alone")
...
# Step 8: Save
joblib.dump({
    'base_models': {'xgb': xgb, 'rf': rf},
    'meta_model': meta_model,
    'combo_type': 'XGB_RF_with_DT_Meta',
    'metrics': combo1_metrics
}, 'hybrid_combo1_xgb_rf_dt_meta.joblib')

print("\n💾 Combo 1 saved as 'hybrid_combo1_xgb_rf_dt_meta.joblib'")