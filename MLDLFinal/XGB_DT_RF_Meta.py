# Hybrid_Combo2_XGB_DT_with_RF_Meta.py
import pandas as pd
import numpy as np
...
print("🚀 HYBRID COMBO 2: (XGBoost + Decision Tree) → Random Forest as Meta")
print("=" * 70)
...
meta_features_train = np.column_stack([xgb_pred_train, dt_pred_train])
meta_features_test = np.column_stack([xgb_pred_test, dt_pred_test])

print(f"Meta features shape: {meta_features_train.shape}")

# Step 5: Train Meta Model (Random Forest)
...
combo2_metrics = calculate_all_metrics(y_test, final_pred, "Hybrid Combo 2")
...
rf_alone = RandomForestRegressor(n_estimators=100, random_state=42).fit(X_train, y_train)
rf_pred_alone = rf_alone.predict(X_test)
rf_metrics = calculate_all_metrics(y_test, rf_pred_alone, "Random Forest Alone")
...
# Step 8: Save
joblib.dump({
    'base_models': {'xgb': xgb, 'dt': dt},
    'meta_model': meta_model,
    'combo_type': 'XGB_DT_with_RF_Meta',
    'metrics': combo2_metrics
}, 'hybrid_combo2_xgb_dt_rf_meta.joblib')

print("\n💾 Combo 2 saved as 'hybrid_combo2_xgb_dt_rf_meta.joblib'")