...
# hybrid_xgb_gb_dt.py
print("="*70)
print("HYBRID MODEL: XGBoost + Gradient Boosting + Decision Tree")
print("="*70)
...
total_r2 = xgb_r2 + gb_r2 + dt_r2
xgb_weight = xgb_r2 / total_r2
gb_weight = gb_r2 / total_r2
dt_weight = dt_r2 / total_r2

hybrid_pred = (xgb_pred * xgb_weight) + (gb_pred * gb_weight) + (dt_pred * dt_weight)
...
plt.title('Confusion Matrix - XGB+GB+DT\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_xgb_gb_dt.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'xgb': xgb_model, 'gb': gb_model, 'dt': dt_model,
            'weights': [xgb_weight, gb_weight, dt_weight]},
           'hybrid_xgb_gb_dt.joblib')