...
# hybrid_xgb_gb_knn.py
print("="*70)
print("HYBRID MODEL: XGBoost + Gradient Boosting + KNN")
print("="*70)
...
total_r2 = xgb_r2 + gb_r2 + knn_r2
xgb_weight = xgb_r2 / total_r2
gb_weight = gb_r2 / total_r2
knn_weight = knn_r2 / total_r2

hybrid_pred = (xgb_pred * xgb_weight) + (gb_pred * gb_weight) + (knn_pred * knn_weight)
...
plt.title('Confusion Matrix - XGB+GB+KNN\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_xgb_gb_knn.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'xgb': xgb_model, 'gb': gb_model, 'knn': knn_model,
            'weights': [xgb_weight, gb_weight, knn_weight]},
           'hybrid_xgb_gb_knn.joblib')