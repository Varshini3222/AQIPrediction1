...
# hybrid_xgb_rf_gb_knn.py
print("="*70)
print("HYBRID MODEL: XGBoost + Random Forest + Gradient Boosting + KNN")
print("="*70)
...
total_r2 = xgb_r2 + rf_r2 + gb_r2 + knn_r2
xgb_weight = xgb_r2 / total_r2
rf_weight = rf_r2 / total_r2
gb_weight = gb_r2 / total_r2
knn_weight = knn_r2 / total_r2

hybrid_pred = (xgb_pred * xgb_weight) + (rf_pred * rf_weight) + (gb_pred * gb_weight) + (knn_pred * knn_weight)
...
plt.title('Confusion Matrix - XGB+RF+GB+KNN\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_xgb_rf_gb_knn.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'xgb': xgb_model, 'rf': rf_model, 'gb': gb_model, 'knn': knn_model,
            'weights': [xgb_weight, rf_weight, gb_weight, knn_weight]},
           'hybrid_xgb_rf_gb_knn.joblib')