...
## hybrid_xgb_rf_dt.py
print("="*70)
print("HYBRID MODEL: XGBoost + Random Forest + Decision Tree")
print("="*70)
...
total_r2 = xgb_r2 + rf_r2 + dt_r2
xgb_weight = xgb_r2 / total_r2
rf_weight = rf_r2 / total_r2
dt_weight = dt_r2 / total_r2

hybrid_pred = (xgb_pred * xgb_weight) + (rf_pred * rf_weight) + (dt_pred * dt_weight)
...
plt.title('Confusion Matrix - XGB+RF+DT\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_xgb_rf_dt.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'xgb': xgb_model, 'rf': rf_model, 'dt': dt_model,
            'weights': [xgb_weight, rf_weight, dt_weight]},
