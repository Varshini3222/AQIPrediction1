...
# hybrid_gb_knn.py
print("="*70)
print("HYBRID MODEL: Gradient Boosting + KNN")
print("="*70)
...
total_r2 = gb_r2 + knn_r2
gb_weight = gb_r2 / total_r2
knn_weight = knn_r2 / total_r2

hybrid_pred = (gb_pred * gb_weight) + (knn_pred * knn_weight)
...
plt.title('Confusion Matrix - Gradient Boosting + KNN\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_gb_knn.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'gb': gb_model, 'knn': knn_model, 'weights': [gb_weight, knn_weight]},
           'hybrid_gb_knn.joblib')