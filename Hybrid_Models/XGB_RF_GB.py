...
# hybrid_xgb_rf_gb.py
print("="*70)
print("HYBRID MODEL: XGBoost + Random Forest + Gradient Boosting")
print("="*70)
...
plt.title('Confusion Matrix - XGB+RF+GB\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_xgb_rf_gb.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'xgb': xgb_model, 'rf': rf_model, 'gb': gb_model,
            'weights': [xgb_weight, rf_weight, gb_weight]},
           'hybrid_xgb_rf_gb.joblib')