...
# hybrid_rf_dt.py
print("="*70)
print("HYBRID MODEL: Random Forest + Decision Tree")
print("="*70)
...
total_r2 = rf_r2 + dt_r2
rf_weight = rf_r2 / total_r2
dt_weight = dt_r2 / total_r2

hybrid_pred = (rf_pred * rf_weight) + (dt_pred * dt_weight)
...
plt.title('Confusion Matrix - Random Forest + Decision Tree\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_rf_dt.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'rf': rf_model, 'dt': dt_model, 'weights': [rf_weight, dt_weight]},
           'hybrid_rf_dt.joblib')