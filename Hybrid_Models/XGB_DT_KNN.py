...
# hybrid_xgb_dt_knn.py
print("="*70)
print("HYBRID MODEL: XGBoost + Decision Tree + KNN")
print("="*70)
...
total_r2 = xgb_r2 + dt_r2 + knn_r2
xgb_weight = xgb_r2 / total_r2
dt_weight = dt_r2 / total_r2
knn_weight = knn_r2 / total_r2

hybrid_pred = (xgb_pred * xgb_weight) + (dt_pred * dt_weight) + (knn_pred * knn_weight)
...
plt.title('Confusion Matrix - XGB+DT+KNN\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_xgb_dt_knn.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'xgb': xgb_model, 'dt': dt_model, 'knn': knn_model,
            'weights': [xgb_weight, dt_weight, knn_weight]},
           'hybrid_xgb_dt_knn.joblib')