...
# hybrid_xgb_rf.py
print("="*70)
print("HYBRID MODEL: XGBoost + Random Forest")
print("="*70)
...
total_r2 = xgb_r2 + rf_r2
xgb_weight = xgb_r2 / total_r2
rf_weight = rf_r2 / total_r2

hybrid_pred = (xgb_pred * xgb_weight) + (rf_pred * rf_weight)
...
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_xgb_rf.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

# Save model
joblib.dump({'xgb': xgb_model, 'rf': rf_model, 'weights': [xgb_weight, rf_weight]},
           'hybrid_xgb_rf.joblib')