# hybrid_gb_dt.py
# Template for all hybrid models
import pandas as pd
import numpy as np
...
# hybrid_gb_dt.py
print("="*70)
print("HYBRID MODEL: Gradient Boosting + Decision Tree")
print("="*70)
...
total_r2 = gb_r2 + dt_r2
gb_weight = gb_r2 / total_r2
dt_weight = dt_r2 / total_r2

hybrid_pred = (gb_pred * gb_weight) + (dt_pred * dt_weight)
...
plt.title('Confusion Matrix - Gradient Boosting + Decision Tree\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_gb_dt.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'gb': gb_model, 'dt': dt_model, 'weights': [gb_weight, dt_weight]},
           'hybrid_gb_dt.joblib')