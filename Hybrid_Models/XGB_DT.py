...
# hybrid_xgb_dt.py
print("="*70)
print("HYBRID MODEL: XGBoost + Decision Tree")
print("="*70)
...
total_r2 = xgb_r2 + dt_r2
xgb_weight = xgb_r2 / total_r2
dt_weight = dt_r2 / total_r2

hybrid_pred = (xgb_pred * xgb_weight) + (dt_pred * dt_weight)
...
plt.title('Confusion Matrix - XGBoost + Decision Tree\nAccuracy: {:.2f}%'.format(accuracy_category))
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.tight_layout()
plt.savefig('confusion_xgb_dt.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n" + "="*70)

joblib.dump({'xgb': xgb_model, 'dt': dt_model, 'weights': [xgb_weight, dt_weight]},
           'hybrid_xgb_dt.joblib')